...
console.log("Task 2:", artists[9].name  = 'Vincent Van Gogh');
...
function listOfNames(array) {
  ...
  const result = newArray.map(item => item.name)
  return result;
}
...
function addArtist(array, name, years, genre, nationality, bio) {
  ...
  //   bio: "Add 1-2 sentences (or use lorem ipsum)",
  // } ;
 newArray.push({"name": name, "years": years, "genre": genre, "nationality": nationality, "bio": bio});
  return newArray
...
function lotsOfArt(artists){
  const newArray = artists.filter((artist) => {return artist.paintings >= 100;});
  const result = newArray.map(item => item.name)
    return result;
  }
...
function artistInfo(artists, name){
  const newArray = artists.filter((artist) => {return artist.name == name;});
  const result = newArray.map(item => item.bio).toString();
  return result;
  }
...
function artistByCountry(artists, nationality){
  const newArray = artists.filter((artist) => {return artist.nationality == nationality;});
  const result = newArray.map(item => item.name)
  return result;
  
  }
  console.log("task 7:", artistByCountry(artists, 'Spanish'));
